  ...
  Widget build(BuildContext context) {
    ...
    return AppScaffold(
      floatingActionButton: Container(
        width: AppSizes.getBaseScale(context) * 55,
        height: AppSizes.getBaseScale(context) * 55,
        child: FittedBox(
      ...
      body: BlocConsumer<HomeCubit, HomeState>(
        ...
        builder: (context, state) => DefaultTabController(
          length: cubit.areasList.length == 1 ? 5 : cubit.areasList.length,
          child: LiquidPullToRefresh(
            ...
            child: CustomScrollView(
              ...
              slivers: [
                  ...
                  child: Column(
                    children: [
                      CarouselSlider.builder(
                        ...
                        itemBuilder: (context, index, realIndex) => cubit
                            ...
                            : CarouselBox(
                                ...
                                image: ApiConstants.kUrl +
                                    cubit.featuredProductsList![index].product!
                                        .mainPhoto!,
                                productInfo:
                                    cubit.featuredProductsList![index].product!,
                              ).p16,
                ...
                SliverToBoxAdapter(
                  child: ButtonsTabBar(
                    backgroundColor: cubit.areasList.length == 1
                    ...
                    onTap: (index) {
                      if (cubit.areasList.length != 1) {
                        cubit.changeTabIndex(index);
                      }
                    },
                    tabs: List.generate(
                      cubit.areasList.length == 1 ? 5 : cubit.areasList.length,
                      (index) => cubit.areasList.length == 1
                          ? Tab(
                              child: Skeletonizer(
                                enabled: cubit.areasList.length == 1,
                                child: BodyTinyText('Some Area'),
                              ),
                            )
                          : MainAppTab(
                              title: cubit.areasList[index].areaName,
                              isSelected: cubit.currentTabIndex == index),
                    ),
                  ),
                ),
                SliverList.builder(
                    itemBuilder: (context, index) => cubit.productsList.isEmpty ||
                        ...
                        : cubit.currentTabIndex == 0
                            ? MainItemBox(
                                    ...
                                    isMyProduct: cubit.productsList[index].userId
                                            .toString() ==
                                        ApiConstants.kUserId,
                                    address: cubit.productsList[index].address!)
                                .p16
                            : cubit.productsList[index].areaId ==
                                    cubit.areasList[cubit.currentTabIndex].id
                                ? MainItemBox(
                                        context: context,
                                        image: ApiConstants.kUrl +
                                            cubit
                                                .productsList[index].mainPhoto!,
                                        title: cubit.productsList[index].title!,
                                        price: cubit.productsList[index].price!,
                                        isMyProduct: cubit
                                            ...
                                            ApiConstants.kUserId,
                                        address:
                                            cubit.productsList[index].address!,
                                        productInfo: cubit.productsList[index])
                                    .p16
                                : SizedBox(),
